...
const seed = async () => {
  ...
  const [engineering, science] = await Faculty.create([
    { name: "Faculty of Engineering", description: "Engineering programmes" },
    { name: "Faculty of Science", description: "Science programmes" }
  ]);

  const [computerScience, softwareEngineering, electricalEngineering] = await Department.create([
  ...
  const [webDev, mobileDev, uiux, aiMl, projectManagement] = await Skill.create([
  ...
  const [fintechInterest, healthTechInterest, edTechInterest] = await Interest.create([
    { name: "FinTech" },
    { name: "HealthTech" },
    { name: "EdTech" }
  ]);

  const [aiCategory, webCategory, iotCategory] = await Category.create([
  ...
  const [attendanceProject, smartFarmProject] = await Project.create([
    {
      ...
      description: "A face recognition based attendance platform for lecture halls.",
    ...
    },
    {
      ...
      description: "An IoT dashboard for tracking soil moisture and crop conditions.",
  ...
  await ProjectMember.create([
    { project: attendanceProject._id, user: ada._id, roleName: "owner", addedBy: ada._id },
    { project: attendanceProject._id, user: tunde._id, roleName: "frontend developer", addedBy: ada._id },
    { project: smartFarmProject._id, user: grace._id, roleName: "owner", addedBy: grace._id }
  ]);
  ...
  const [task1, task2] = await Task.create([
    {
      ...
      progress: 45,
      activityLog: [{ action: "task_created", actor: ada._id, note: "Initial task seeded" }]
    },
    {
      ...
      activityLog: [{ action: "task_created", actor: grace._id, note: "Initial task seeded" }]
    }
  ]);
  ...
  const [message1] = await Message.create([
    {
      ...
      project: attendanceProject._id,
      sender: ada._id,
      content: "Let us split the attendance module into recognition and reporting.",
      deliveredTo: [{ user: ada._id }, { user: tunde._id }],
      readBy: [{ user: ada._id }]
    }
  ]);

  await Conversation.findByIdAndUpdate(attendanceConversation._id, { lastMessage: message1._id });
  ...
  await Notification.create([
    ...
    },
    {
      ...
      data: { conversationId: attendanceConversation._id, projectId: attendanceProject._id }
    }
  ]);
  ...
  await ProjectInvitation.create({
    ...
    invitedBy: ada._id,
    message: "Your embedded systems experience would help with the camera hardware setup.",
